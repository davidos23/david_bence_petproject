---
title: "Szívbetegség előrejelzése - Ensemble"
...
Utolsó módszerként egy heterogén stacking ensemble modellt próbálunk megépíteni.
Ennek összetevőiként az eddigi legjobbnak bizonyult neurális háló, és random forest modelleket fogjuk használni, illetve harmadik összetevőként beleveszünk egy egyszerű logit modellt.


Elsőnek tekintsük meg tehát, hogyan teljesít egy logit modell ezen a sokaságon.
...
A GVIF mutató sehol sem túl magas (nincs 2-t meghaladó):
```{r}
vif(logit_all)
```

Kidobjuk a magas p-értékkel rendelkező változókat (marad: nem, mellkasi fájdalom fajtája, koleszterol szint, vércukor szint, fizikai aktivitás indukálta angina, sinus tachycardia csúcsossága, mértéke), és lefuttatjuk K = 100 db-szor a becslést, majd megnézzük a mutatóinkat:
```{r}
K=1000
...
Nem túl erősek a mutatók (kb. 60%-os pontosság).

Készítsünk egy stacking ensemble modellt (manuálisan)!
__________________________________________________________________________________
Elsőnek 3 részre bontjuk a az adatainkat:
  - alapmodell-tanító halmaz (50%)
  - alapmodell-teszt és egyben metamodell-tanító halmaz (30%)
  - metamodell-teszt halmaz (20%)

Később, a legjobb RF és ANN modellel való összehasonlításakor mindig ezt a 25% adatot fogjuk használni.
```{r}
#Első felosztás
...
Neurális háló számára szükséges ezeknek a normált verziója is:
```{r}
...
Külön-külön tanítjuk a 3 modellt az első 50%-on:
```{r}
...
Alapmodell becslések a középső 30%-on
```{r}
#ANN becslések
...
Futassuk le K-szor, és nézzük az átlagos mutatókat:

Ám előtte rögzítsük a K db világállapotot. Ezek mellett fogjuk összehasonlítani a másik kettő modellhez.
