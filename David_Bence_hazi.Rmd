...
K = 100 db különböző split futtatása, n=1:4 neuronos rejtett rétegekre:
```{r}
...
average_metric_1layer %>% as.data.frame()
...
for (neuronszam1 in 1:n1){
  ...
  for (i in 1:K){
    ...
    #Neurális háló tanítása
    tryCatch({
      ANN_looped <- neuralnet(as.factor(HeartDisease)~., data = heart_kesz_train_loop,hidden=c(neuronszam1,neuronszam2),linear.output = FALSE,stepmax=100000,learningrate = 0.15)
...
average_metrics_2layer <- rbind(average_metrics_11,average_metrics_12,average_metrics_21,average_metrics_22,average_metrics_31,average_metrics_32)
average_metric_2layer %>% as.data.frame()
