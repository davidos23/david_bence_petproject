...
Töltsük be a szükséges packagekat.
```{r}
library(randomForest)
...
```
 ...
 (fontos: futásonként változhat az optimális érték, így rögzítsük a seed-et)
```{r}
set.seed(1)
...
Gyártsunk le egy teszt adatot és nézzük meg az eredményt:
```{r}
test_data = heart_failure[1:300,]
prediction_1 = predict(vegso,test_data)
confusionMatrix(prediction_1, as.factor(test_data$HeartDisease))
```
